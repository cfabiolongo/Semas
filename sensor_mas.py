...
class worker(Agent):
    def main(self):
        +TASK(X)[{'from': Y}] >> [show_line("Worker received task ", X, " from ", Y)]

class worker2(Agent):
    def main(self):
        +TASK(X)[{'from': Y}] >> [show_line("Worker2 received task ", X, " from ", Y)]
...
class main(Agent):
    def main(self):
        ...
        +TASK(X) / DUTY(Y) >> [+TASK(X)[{'to':'worker'}], +TASK(X)[{'to':'worker2'}]]
